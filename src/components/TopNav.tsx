...
export const TopNav: React.FC<TopNavProps> = ({ onOpenNotifications, onOpenAdminProfile }) => {
  ...
  const navItems = [
    ...
    { id: 'services', label: 'Servis & Antrean', icon: CalendarClock },
    { id: 'couriers', label: 'Kurir Jemput-Bola', icon: Navigation },
    { id: 'inventory', label: 'Suku Cadang', icon: Boxes, alertCount: lowStockCount },
    { id: 'employees', label: 'Karyawan & Akses', icon: Users, isLocked: !isAdmin },
    { id: 'reports', label: 'Laporan Bulanan', icon: FileSpreadsheet },
    { id: 'rad', label: 'Metode RAD', icon: Cpu }
  ] as const;
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="hidden sm:block text-left leading-tight">
                <div className="font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-1">
                  <span className="max-w-[100px] truncate">{currentEmployee.name}</span>
                  {currentEmployee.role === 'Super Admin' ? (
                ...
                <div className="text-[10px] text-sky-700 dark:text-sky-400 font-mono font-medium">{currentEmployee.role}</div>
              </div>
            ...
            {roleDropdownOpen && (
              ...
              >
                <div className="border-b border-slate-100 pb-2 mb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-bold border ${
                      ...
                      {currentEmployee.role === 'Super Admin' && <Crown className="h-3 w-3 text-amber-600 fill-amber-400" />}
                      {currentEmployee.role}
                    </span>
                  </div>
                  <div className="text-[11px] text-slate-500 mt-0.5">{currentEmployee.email}</div>

                  {/* Open Admin Profile Button */}
                  <div className="mt-2 flex items-center gap-1.5">
                    {onOpenAdminProfile && (
                      <button
                        ...
                        className="flex-1 rounded-lg border border-sky-300 bg-sky-50 hover:bg-sky-100 py-1.5 px-2 text-[11px] font-bold text-sky-800 text-center transition-colors flex items-center justify-center gap-1"
                      >
                        <Settings className="h-3 w-3 text-sky-600" />
                ...
                {/* Direct 1-Click "Jadikan Saya Admin" if not already Super Admin */}
                {currentEmployee.role !== 'Super Admin' && (
